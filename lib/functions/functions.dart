...
import '../pages/trip screen/booking_confirmation.dart';
import '../pages/trip screen/map_page.dart';
import '../pages/trip screen/review_page.dart';
import '../pages/referralcode/referral_code.dart';
import '../styles/styles.dart';

//languages code
bool cancellingUser = false;
  String cancelReason = '';
  String cancelCustomReason = '';
    String cancellingError = '';
...
//base url
String url =
    'https://15deabril.macrobyte.site/'; //add '/' at the end of the url as 'https://url.com/'
String mapkey = Platform.isAndroid
    ? 'AIzaSyA0Jl2tjqouHdhCzNF8nogi381QHIq9c_w'
    : 'AIzaSyA0Jl2tjqouHdhCzNF8nogi381QHIq9c_w';
String mapType = '';

//dibujado
// Future<List<fmlt.LatLng>> getRouteFromGraphHopper(double startLat, double startLng, double endLat, double endLng) async {
//   final String apiKey = '55cb099e-7d08-457a-b176-52d07bc27a19'; // Tu clave API de GraphHopper
...
//     // Convertir las coordenadas a una lista de LatLng
//     List<fmlt.LatLng> polylinePoints = coordinates.map((point) {
//       return fmlt.LatLng(point[1], point[0]); // LatLng(latitude, longitude)
//     }).toList();
...
// Future<void> aceptarBocina(String requestId) async {
//   Mylogger.print(requestId);
//   const url = 'https://15deabril.macrobyte.site/api/bocina';
...
//     if (response.statusCode == 200) {
//       print('Bocina actualizada exitosamente');
//     } else {
//       print('Error al actualizar la bocina: ${response.body}');
//     }
//   } catch (error) {
//     print('Error: $error');
//   }
...
Future<void> aceptarBocina(String requestId) async {
  const url = 'https://15deabril.macrobyte.site/api/boton-cliente/confirmacion';
  ...
  try {
    final response = await http.post(
      ...
      }, // Cambié el content-type a JSON
      body: jsonEncode(
          {'request_id': requestId}), // JSON para enviar el request_id
    );

    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      print('Confirmación registrada correctamente: ${data['message']}');
    } else {
      print('Error al registrar la confirmación: ${response.body}');
    }
  } catch (error) {
    print('Error: $error');
  }
...
registerUserWithNumber(String userName, String number, String coCode) async {
  final emailRandom = const Uuid().v4();
  bearerToken.clear();
  dynamic result;
  try {
...
Future<bool> existsUser(String email) async {
  print("ejecutando exist user");
  try {
    var response = await http.post(Uri.parse('${url}api/v1/user/validate-mobile-for-login'), 
    body: {
      "email": email,
    });
    print(response.body);
    if (response.statusCode == 200) {
      var val = jsonDecode(response.body)['success'];
      if (val == true) {
        /// login por email en el sistema
        var check = await userLogin(email, 1, email, false);
        if (check == true) {
...
Future<bool> loginWithWhatsapp(String number) async {
  bool? resp;
  try {
    final response = await http.get(
      Uri.parse('${url}api/cliente/validar/?telefono=$number'),
    );
...
getUserDetails({id}) async {
  ...
  if (_lastExecutionTime != null &&
      DateTime.now().difference(_lastExecutionTime!) < debounceDuration) {
    // Si se llamó hace menos de 2 segundos, no ejecuta el método
    return;
  }
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonDecode(response.body)['message'] == 'no drivers available') {
        ...
        if (response.statusCode == 403) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const OnGoingRides()),
          );
        }
...
cancelRequest([dynamic requestID]) async {
  ...
  try {
    var response = await http.post(
      ...
      body: jsonEncode(
        {'request_id': requestID ?? userRequestData['id']},
      ),
    );
    if (response.statusCode == 200) {
...
cancelReasons(reason) async {
  dynamic result;
  try {
    var response = await http.get(
      Uri.parse('${url}api/v1/common/cancallation/reasons?arrived=before&transport_type=both'),
      headers: {
        'Authorization': 'Bearer ${bearerToken[0].token}',
        'Content-Type': 'application/json',
      },
...
// Definir chatList como ValueNotifier
ValueNotifier<List<dynamic>> chatListNotifier =ValueNotifier<List<dynamic>>([]);
...
Future<void> getCurrentMessages() async {
  ...
  try {
    var response = await http.get(
      Uri.parse('${url}api/v1/request/chat-history/$ri'),
      headers: {
        'Authorization': 'Bearer ${bearerToken[0].token}',
        'Content-Type': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['success'] == true) {
        List<dynamic> newMessages = responseData['data'];

        // Verifica si hay nuevos mensajes para actualizar chatListNotifier
        // if (newMessages.length != chatListNotifier.value.length || newMessages.any((msg) => !chatListNotifier.value.contains(msg))) {
          chatListNotifier.value = List.from(newMessages);
        // }
      }
    } else if (response.statusCode == 401) {
      debugPrint('Token inválido, cierre de sesión necesario.');
      // Implementa lógica de cierre de sesión si es necesario
    } else {
      debugPrint('Error en la solicitud: ${response.body}');
    }
  } catch (e) {
    if (e is SocketException) {
...
sendMessage(chat) async {
  ...
  try {
    var response = await http.post(Uri.parse('${url}api/v1/request/send'),
        ...
        body: jsonEncode({'request_id': ri, 'message': chat}));
    if (response.statusCode == 200) {
...
//message seen
DateTime? _lastExecutionTimeSeen;

messageSeen() async {
  const debounceDurationSeen = Duration(seconds: 2);

  // Verifica si el método fue llamado antes y si han pasado al menos 2 segundos
  if (_lastExecutionTimeSeen != null &&
      DateTime.now().difference(_lastExecutionTimeSeen!) <
          debounceDurationSeen) {
    // Si se llamó hace menos de 2 segundos, no ejecuta el método
    return;
  }

  // Actualiza el tiempo de la última ejecución
  _lastExecutionTimeSeen = DateTime.now();
